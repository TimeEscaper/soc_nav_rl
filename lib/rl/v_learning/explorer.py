...
class Explorer:
    def __init__(self,
                 env: Union[VecEnv, SubprocVecEnv],
                 device: str,
    ...
    def update_target_model(self, target_model: nn.Module):
        self._target_model = copy.deepcopy(target_model)
    ...
    def run_k_episodes(self,
            ...
            for i in range(self._env.num_envs):
                ...
                if done[i]:
                    ...
                    if update_memory and done_reason in ("success", "collision"):
                        self._update_memory(states[i], rewards[i], imitation_learning)
        ...
        print(f"Total replay memory size: {len(self._memory)}")
    ...
    def _update_memory(self, states, rewards, imitation_learning=False):
        if self._memory is None or self._gamma is None:
        ...
        for i, state in enumerate(states):
            ...
            if imitation_learning:
                ...
                value = sum([pow(self._gamma, max(t - i, 0)) * reward
                ...
                if i == len(states) - 1:
                    ...
                    gamma_bar = pow(self._gamma, 1.)
                    next_state = {k: torch.Tensor(v).unsqueeze(0).to(self._device) for k, v in next_state.items()}
                    with torch.no_grad():
                        value = self._target_model(next_state).item()
                    value = reward + gamma_bar * value
            value = torch.Tensor([value]).float()

            self._memory.push((state, value))
