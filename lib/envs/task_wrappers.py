...
class PredictionEnv(AbstractTaskWrapper):
    ...
    def _build_observation(self) -> Dict[str, np.ndarray]:
        ...
        pred_mean_rl = obs_pred_mean[:, :(self._rl_horizon + 1), :]
        pred_cov_rl = obs_pred_cov[:, :self._rl_horizon, :, :]
        pred_mean_rl = pred_mean_rl - robot_pose[:2]

        pred_mean_rl = np.einsum("ij,mnj->mni", rotation_matrix, pred_mean_rl)
        pred_cov_rl = np.einsum("ij,mnjk->mnik", rotation_matrix, pred_cov_rl)
        pred_cov_rl = np.einsum("mnij,jk->mnik", pred_cov_rl, rotation_matrix.T)

        return {
            "robot": obs_robot,
            "pred_mean": obs_pred_mean,
            "pred_cov": obs_pred_cov,
            "pred_mean_rl": pred_mean_rl,
            "pred_cov_rl": pred_cov_rl,
            "visibility": peds_vis
        }
